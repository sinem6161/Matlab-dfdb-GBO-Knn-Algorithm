...
function [] = dfdb_GBO_Case3()
%% Initialization
[nP, nV, MaxIt, lb, ub] = problem_terminate();
    ...
    for i=1:nP     
       ...
       ro = alpha.*(2*rand-1);ro1 = alpha.*(2*rand-1);        
        ...
        if rand<pr           
            ...
            if u1<0.5
                  if(rand()<0.4)
                Xnew = Xnew + f1.*(u1.*Best_X-u2.*Xp)+f2.*ro.*(u3.*( X2-X(index,:))+u2.*(X(r1,:)-X(r2,:)))/2;     
                  else
                Xnew = Xnew + f1.*(u1.*Best_X-u2.*Xp)+f2.*ro.*(u3.*(X2-X1)+u2.*(X(r1,:)-X(r2,:)))/2;     
                 end
            else
                 if(rand()<0.8)
                Xnew = Best_X + f1.*(u1.*X(index,:)-u2.*Xp)+f2.*ro.*(u3.*(X2- X1)+u2.*(X(r1,:)-X(r2,:)))/2;   
                 else
